  ...
  return (
    ...
    <> 
      {roomList.data?.map((room, index) => {
        return (
            <List onClick={()=>navigate(`/chat/${room.roomNo}`, 
            {state:{index: index, targetNo: room.chatRoomMembers[1].memberNo, targetNick: room.chatRoomMembers[1].nick}})}
            key={room.roomNo}
            >
              <span>{nick===room.chatRoomMembers[0].nick?
